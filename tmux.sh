...
if [ "$1" = "-h" ] || [ "$1" == "--help" ]; then
    printf "\n"
    printf "\033[1m  t - tmux session manager with Nix shell support\033[0m\n"
    printf "\n"
...
get_fzf_find_binding() {
    ...
    local fzf_find_binding_default='ctrl-f:change-prompt(find> )+reload(find ~ -maxdepth 6 -type d)'
    if [ "$TMUX_RUNNING" -eq 0 ]; then
...
fzf_border_label_default=' t - tmux session manager '
...
if [ -z "$SESSION" ]; then
    ...
    if check_nix_shell "$RESULT"; then
        tmux new-session -d -s "$SESSION" -c "$RESULT"
        create_nix_shell_window "$SESSION" "$RESULT" "main"
    else
        tmux new-session -d -s "$SESSION" -c "$RESULT" "zsh"
    fi
fi
